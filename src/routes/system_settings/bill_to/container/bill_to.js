...
class BillTo extends React.Component{
  ...
  render(){
    ...
    const columns = [
      {dataIndex:_billT.id,width:100,render: text => <a onClick={this.billDetails.bind(this,text)}>{text}</a>,},
      {dataIndex:_billT.nameEN},
      {dataIndex:_billT.nameCN},
      {dataIndex:_billT.country},
      {dataIndex:_billT.city},
      {dataIndex:_billT.address,render: (text, record) => getToolTip(text,20)},
      {dataIndex:_billT.postCode},
      {dataIndex:_billT.status,render:text=><span style={text==1?{color:'green'}:{color:'red'}} >{text==1?formatMessage({id:'normal'}):formatMessage({id:'disabled'})}</span>,width:60},
      {dataIndex:_billT.operation,width:100,
        render: (text, record) => (
    ...
    const formColumns = [
      ...
      {dataIndex:_billT.address,colStyle:{height:'100%'},option:this.getRequiredMessage(formatMessage({id:'required_fields'}))},
      {dataIndex:_billT.postCode},
    ].map(
      item=>({
        ...item,
    ...
    const columnMap=column=>{
      ...
      return (
        <Col key={column.dataIndex} span={column.span || 24 } className='payment-item'>
          <span  className="payment-label" style={{fontWeight:'bold'}}>{formatMessage({id:`bill_to_${column.dataIndex}`})}</span>
          <span span={14}  className="payment-value" style={bold&&{fontWeight:"bold"}}>{
