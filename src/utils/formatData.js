  ...
  function sub(t,n) {
    ...
    for(let i=m; i<t.length; i++) {
      if(t.substr(0, i).replace(r, "mm").length>=n) {
        return <span>{t.substr(0, i) +" ... "}<Icon type="question-circle-o" /></span> }
    } return t;
  };

  return <span>
          <div style={{display:'inline-block',marginRight:'15px'}}><Tooltip title={<span>{text}</span>}>{sub(text,showChar)}</Tooltip></div>
      </span>
}
