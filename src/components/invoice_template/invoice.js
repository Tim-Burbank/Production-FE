...
class Invoice_template extends React.Component {
  ...
  render(){
    ...
    const renderSimpleColumn =(column,data)=>{
      ...
      return (
        <Col key={column.dataIndex} span={column.span || 12 } className='payment-item' style={column.title?{textAlign:'center'}:{textAlign:'right',paddingRight:'93px',borderRight:0}}>
          <span  style={column.title&&{fontWeight:'bold'}}>{text}</span>
        </Col>
      )};


    const renderColumn =(column,data,data2,_style={})=>{
      let text
      if(data&&data2){
      ...
      return (
        <Col key={column.dataIndex} span={column.span || 24 } className='payment-item' style={{display:'flex',..._style}}>
          <span   className="payment-label" style={{fontWeight:'bold'}}>{this.toUpper(column.title)}</span>
      ...
      <Card {...props}  onDoubleClick={()=>this.setState({editIndex:-1})} >
        ...
        <Row className="info">
          <Row className="item">
            ...
            <Row className="payment-read" style={{borderLeft:0}}>
              {leftColumn.map(v=>renderColumn(v,clientPOInfo,invoicesInfo,{borderRight:0}))}
            </Row>
          </Row>
          <Row className="item">
            <p style={{fontWeight:'bold'}}>INVOICE INFORMATION</p>
            <Row className="payment-read">
              {rightColumn.map(v=>renderColumn(v,invoicesInfo,clientPOInfo,{borderRight:0}))}
            </Row>
          </Row>
        </Row>
        <Row >
        ...
        <Row className=" bankI-info">
          ...
          {placedToInfo&&
          <Row className="payment-read" style={{marginBottom:0}}>
